...
int main() {
  ...
  TFile * output = TFile::Open("gammaA.root","update");
  TH1D * mult = new TH1D("mult_5_eta2p5",";N_{ch};Normalized to Unity",100,0,200);
  TH1D * dNdEta = new TH1D("dNdEta_5_eta2p5",";#eta;#frac{1}{N_{evt}}#frac{dN_{ch}}{d#eta}",48,-6,6);
  ...
  for (int iEvent = 0; iEvent < nEvent; ++iEvent) {
    ...
    for (int i = 0; i < pythia.event.size(); ++i)
      if (pythia.event[i].isFinal() && pythia.event[i].isCharged()){
        if(TMath::Abs(pythia.event[i].eta()) > 2.5 ) continue;
        if(TMath::Abs(pythia.event[i].pT()) < 0.1 ) continue;

        ++nCh;
        dNdEta->Fill( pythia.event[i].eta(),weight);
      }
    mult->Fill(nCh,weight);
  ...
  mult->Scale(1.0/(double)weightSum);
  dNdEta->Scale(4.0/(double)weightSum);
  output->Write();
  output->Close();
