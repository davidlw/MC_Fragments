...
int main() {
  ...
  // Histograms.
  TFile * output = TFile::Open("epem.root","update");
  TH1D * mult = new TH1D("mult_91",";N_{ch};Normalized to Unity",100,0,200);
  TH1D * dNdEta = new TH1D("dNdEta_91",";#eta;#frac{1}{N_{evt}}#frac{dN_{ch}}{d#eta}",48,-6,6);
  Hist nCharge("charged multiplicity", 100, -0.5, 99.5);
  //Hist dNdEta("dNdEta", 48, -6, 6);
  ...
  for (int iEvent = 0; iEvent < 300000; ++iEvent) {
    ...
    for (int i = 0; i < pythia.event.size(); ++i)
      if (pythia.event[i].isFinal() && pythia.event[i].isCharged()){
        if(TMath::Abs(pythia.event[i].eta()) > 6 ) continue;
        if(TMath::Abs(pythia.event[i].pT()) < 0.1 ) continue;

        ++nCh;
        dNdEta->Fill( pythia.event[i].eta());
      }
    mult->Fill(nCh);
    //nCharge.Fill( nCh );
  ...
  pythia.stat();
  //cout << nCharge << dNdEta << spheri << linea << thrust << oblateness
  //     << sAxis << lAxis << tAxis
  //     << nLund << nJade << nDurham
  ...
  mult->Print("All");
  dNdEta->Print("All");
  int nEntries = mult->GetEntries();
  mult->Scale(1.0/(double)nEntries);
  dNdEta->Scale(4.0/(double)nEntries);
  output->Write();
  output->Close();
  // Done.
  return 0;
