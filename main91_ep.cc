...
int main() {
  ...
  TFile * output = TFile::Open("ep.root","update");
  TH1D * mult = new TH1D("mult_920_27p5_etaLT2",";N_{ch};Normalized to Unity",100,0,200);
  TH1D * dNdEta = new TH1D("dNdEta_920_27p5_etaLT2",";#eta;#frac{1}{N_{evt}}#frac{dN_{ch}}{d#eta}",48,-6,6);
  ...
  for (int iEvent = 0; iEvent < nEvent; ++iEvent) {
    ...
    for (int i = 0; i < pythia.event.size(); ++i)
      if (pythia.event[i].isFinal() && pythia.event[i].isCharged()){
        if(TMath::Abs(pythia.event[i].eta()) > 2 ) continue;
        if(TMath::Abs(pythia.event[i].pT()) < 0.1 ) continue;
        if(pythia.event[i].id()==11 && pythia.event[i].e()>1) continue;         
